  ...
  if (request.status >= 200 && request.status < 400) {
    ...
    function createItemTemplate(item) {
      function colorClasses() {
        return item.colors
          .map(color => {
            return `<span class="color ${color}">${color
              .charAt(0)
              .toUpperCase()}${color.slice(1)}</span>`;
          })
          .join(' ');
      }
      return `
        <ul class="device-wrapper">
          ...
          <li class="pic"><img src="${
            item.picture
          }" alt="device's picture"></li>
          <li><span class="parameter">Weight:</span> <span class="data">${
            item.weight
          }</span></li>
          <li><span class="parameter">Size:</span> <span class="data">${
            item.size
          }</span></li>
          <li><span class="parameter">Colors:</span><span class="data">${colorClasses()}</span></li>
          <li><span class="parameter">RAM:</span> <span class="data">${
            item.ram
          }</span></li>
          <li><span class="parameter">Antutu:</span> <span class="data">${
            item.antutu
          }</span></li>    
    ...
    function addToCompare(el) {
      ...
      const html_to_inject = `
        <tr>${Array.from(span)
          ...
          .join(
            ''
          )}<td><button type="button" class="btn remove">Remove</button></td></tr>
      `;
    ...
    const load = function() {
      ...
      arrayBtns.forEach(btns => {
        btns.addEventListener('click', addToCompare);
      });
      ...
      tableBody.addEventListener('click', function(elem) {
        ...
        const nameFromTable =
          elem.target.parentElement.parentElement.firstElementChild.textContent;
        arrayBtns.forEach(btns => {
          const nameFromList = btns.parentElement.parentElement.querySelector(
            '.name .data'
          ).textContent;
    ...
    document.getElementById('app').innerHTML = `
      ...
      <div class="lists">${products.map(createItemTemplate).join('')}</div>
